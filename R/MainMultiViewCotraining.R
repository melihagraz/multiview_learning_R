...
setwd("/Users/melihagraz/Documents/GitHub/multiview_learning_R/outputMulti")
n_NEG = 3 # number of negative classes we are selecting
n_POS = 1 # number of positive classes we are selecting

 
# Naive Bayes Medical Selected
Co_res1 <- MultiViewCoTraininig(lab, unlab,  method = "nb",  train_prob  = 0.8,
                             n_subPool   = 75, n_iteration = 30, imbalanced  = TRUE,
                             feature_sel = TRUE, n_neg = n_NEG, n_pos = n_POS,
                             n_fold = 5, seed=123 )
Co_res1$confusion_matrix_v1
Co_res1$confusion_matrix_v2

res1NB_MS2<- matrix( c(Co_res1$first_last[1:3,7],  Co_res1$first_last[1:3,8],
