...
CoTrain_cv_errorBar<-function(lab, unlabeled, method = c("nb", "rf"), train_prob = 0.8, 
                              n_subPool=75, n_iteration=30,  imbalanced=TRUE, feature_sel=TRUE,
                              n_neg=7, n_pos=1, n_fold=5, seed=NULL, ...){
  ...
  #-------------------------------------------------------------------------------------------------- NULL dataframes
  SensCV_v1<-data.frame(); SensCV_v2<-data.frame() ; SpecCV_v1<-data.frame(); SpecCV_v2<-data.frame()
  NPVCV_v1<-data.frame() ; NPVCV_v2<-data.frame() ; PPVCV_v1<-data.frame() ; PPVCV_v2<-data.frame()
  
  AccCV_v1<-data.frame() ; AccCV_v2<-data.frame() ;F1CV_v1<-data.frame()  ; F1CV_v2<-data.frame()
  BalAccCV_v1<-data.frame() ; BalAccCV_v2<-data.frame()
  
  TP_v1<-data.frame() ;  TP_v2<-data.frame() ; TN_v1<-data.frame() ;  TN_v2<-data.frame()
  FP_v1<-data.frame() ; FP_v2<-data.frame() ;FN_v1<-data.frame() ; FN_v2<-data.frame()
  ...
  for (i_cv in 1:n_fold) {
    ...
    train_view1<-view1_lab[-testIndex, ] ; test_view1 <-view1_lab[ testIndex, ]
    train_view2<-view2_lab[-testIndex, ] ; test_view2 <-view2_lab[ testIndex, ]
    ...
    #---------------------------------------------------------------------------- imbalanced
    #balanced_Co <- function(train_view1, train_view2, seed = NULL, ...)
    if(!imbalanced){
  ...
  plot_Co(      AccCV_fin, name = "Sensitivity" , feature_sel = feature_sel,
                method =  method, format = ".pdf") 
  # F1
  ...
  plot_Co(     F1CV_fin, name = "Sensitivity" , feature_sel = feature_sel,
               method =  method, format = ".pdf") 
  ...
  plot_Co(     BalAccCV_fin, name = "Sensitivity" , feature_sel = feature_sel,
               method =  method, format = ".pdf") 
  #-----------------------------------------------------------------------------TP TN 
  # TP
  ...
  plot_Co(     TP_fin, name = "Sensitivity" , feature_sel = feature_sel,
               method =  method, format = ".pdf") 
  # TN 
